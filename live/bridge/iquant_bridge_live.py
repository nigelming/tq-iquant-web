...
def _flush_quote_summary():
    ...
    _summary_since = time.time()
    print("[BRIDGE-LIVE] quote %d fetches ok  %s" % (total, "  ".join(parts)))
...
def _log_order(oid, op, code, volume, result, dup=False):
    ...
    Replaces the old two-line output (a pre-passorder [BRIDGE-LIVE] order line plus a
    raw [AUDIT-LIVE] JSON dump) with a single line: oid + direction + code + size +
    ...
    if dup:
        tail = "dup " + tail
    print("[BRIDGE-LIVE] ORDER %s %s %s x%s %s"
...
def _confirm_order_arrival(remark, account):
    ...
    *** DUAL-BRIDGE MIRROR *** -- identical logic must exist in
    iquant_bridge.py _confirm_order_arrival. Hand-sync on any change.
    """
...
def _do_place(params):
    ...
    try:
        ...
        result = fn(op_type, 1101, account, code, pr_type, float(price), float(volume),
                    "iquant_bridge_live", 2, remark, _CTX)
...
def _fetch_quote(code, period, count):
    ...
    try:
        ...
        _throttled_log(("empty", code, period),
                       "[BRIDGE-LIVE] xtdata empty %s %s count=%d" % (code, period, count))
    except Exception as e:
        _throttled_log(("fail", code, period),
                       "[BRIDGE-LIVE] xtdata FAIL %s %s: %s" % (code, period, e))
    ...
    try:
        ...
        _throttled_log(("ctx", code, period),
                       "[BRIDGE-LIVE] ContextInfo FAIL %s %s: %s" % (code, period, e))
        return None
