...
- **ADD 加仓阈值特殊值**：`add_position_threshold = -1` 表示跳过回撤检查，只要有 ADD 信号即加仓（含上涨/大涨）；`max_add_count` 加仓次数上限仍生效。正常取值 1%~20%（逢跌加仓，现价较成本下跌 ≥ 阈值才加）。0 表示"不涨即加"（drop ≥ 0）。风控优先级仍高于 ADD（涨到止盈点先止盈清仓，不加）
