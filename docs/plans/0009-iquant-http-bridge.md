...
### 5.2 bar 完成检测 — 相对变化法（不依赖任何绝对时钟）
- DataFrame 的 `stime` 是 yyyymmddHHMMSS 的 **bar 结束时间**（如 10:08:00 = 10:07–10:08 那根结束）；`time` 是毫秒/秒时间戳。`parse_bar_time` 优先 `stime`（14 位串），次选 `time`（按 Asia/Shanghai +8 显式转换，不依赖本机时区），兼容验证脚本观察到的字段名。
- **★ 不用 `stime <= now`，不用 iQuant 时间，不用本机时间 ★**：绝对时钟比较依赖两个不同时钟（bar 时钟 + Core 时钟），Core 部署在 UTC → 全当进行中哑火；本机时钟跑偏 → 漏信号/未来函数。后果不可预知且静默。
- **相对变化判定**：一根 bar 不再是「该股票最新一根」时，它就完成了。
  ```
  第 N 次拉取:  [..., 10:08, 10:09]           ← 10:09 最新(进行中)
  第 N+1 次:    [..., 10:08, 10:09, 10:10]   ← 10:10 出现，10:09 退居第二 → 10:09 完成，触发
  ```
  只比较两次拉取的 bar `stime` 相对变化，**不碰 `now`/服务器时间/本机时间**。Core 部署任何时区、本机/桥时钟跑偏，均不影响判定。
- **多股票按 code 独立判定完成**：每只股票有各自的 latest，该股票的 bar < 其 latest 才算完成。不用全局 max stime —— 否则快股票的进度会把慢股票的「最新 bar」误判为已完成。每 code 独立维护 `last_completed`，避免快股票把全局水位推高导致慢股票漏触发。同一时间戳、各股票已完成的 bar 合并为一根 `BarEvent.stocks`。
- **首次拉取建立基线不触发**（实盘启动不回放历史 bar，那是回测的事）；之后每次触发「上次之后新完成」的 bar，按时间排序逐根触发，推进 `last_completed`。
- 跳 bar（拉取间隔漏中间）：`stime ∈ (last_completed, cur_latest)` 区间内新完成 bar 全部触发，不漏。

### 5.3 数据流
```
Core 每 15s 调 GET /quote?period=1m&count=10
  → 桥读缓存（每秒刷新）返回最新 10 根 1m bar
  → BarPoller 用相对变化判定：最新 bar = 进行中(不触发)，比上次新出现的已完成 bar → 触发
5m 同理直接拉（原生周期），无需聚合。
```
...
| 2 | 最后一根是否进行中 | ✅ `stime` 是 bar 结束时间，最新一根为进行中。**判定方式改为相对变化**（见 §5.2）：一根 bar 不再是最新即完成，不依赖 `now`/绝对时钟（避免部署时区与本机时钟漂移导致哑火/未来函数） |
