...
| B6 | 多 session 并发限制 | ⚠️规则/❌代码 | 🧠 | **限制同一时刻全局只跑 1 个实盘 session**(2026-08-07 定)。简化隔离,避免多引擎争抢桥单线程 + 持仓归属混乱(Q1 未解)。**代码缺口**:`live.py:178` 现状 `if session_id in _ENGINES` 只防**同一 session 重复 start**(返回 running),**不防全局多个不同 session 并跑** → 待改成 `_ENGINES` 非空即拒绝任何新 start(返回错误提示已有 session 在跑) | ✅ 全局限 1 个;⚠️ `live.py:178` 只防重复 start 不防全局,待改 |
...
| F8 | 成交价近似 | ⚠️已知 | — | 用 `bar.close`,prType=14 实际是盘口一档价。切片5 回填修正(→ design §5.6) | — |
| F9 | 佣金/印花税 | ⚠️已知 | — | 首期 0,真实成本从 /deals 回报取 | — |
| F10 | 落库 `status=accepted` | ⚠️ | ✅ | 应拆 `submitted`,切片5(→ design §5.7) | （待确认） |
...
| # | 细节 | 状态 | 方式 | 说明(→ 参照) | 确认结论 |
|---|---|---|---|---|---|
| G1 | 订单状态机 | ❌ | ✅ | submitted→partial→filled/rejected(→ design §7.1)。**提交时序要求(2026-08-07 补,从 I4 崩溃分析定)**:`_persist_trade` 改成**先写 `LiveOrder(status=submitted)` + commit,再(异步)发 passorder**——确保崩在 passorder 已发、未确认窗口时 DB 至少有 submitted 记录,供 I4 查未完结挂回。现状 `_persist_trade` 是 place_order 后才写 LiveOrder+LiveTrade 一起 commit(live_engine.py:333-364),崩在 commit 前 → DB 无记录但券商可能已成交 → 背离。submitted 阶段**不 apply_trade、不写 LiveTrade**(回填确认成交才写) | （待实现,含提交时序改造） |
| G2 | 主循环轮询 /deals | ❌ | ✅ | 已定:主循环轮询(已选)。每轮查未完结 LiveOrder(→ design §7.2) | （待确认） |
...
| G6 | **拒单/部分成交修正** | ❌ | ✅ | 当前按全量成交记账,拒单需反向退回(加回现金/持仓/还原 _lots)。`apply_trade` 只有正向,需补反向或重放修正(→ design §7.4) | （待确认） |
| G7 | 桥状态并入 session API | ❌ | ✅ | 0009 §11.5 要求并入 `/sessions/{id}` 详情(→ design §1.4) | （待确认） |
...
| # | 细节 | 优先级 | 依赖 |
|---|---|---|---|
| C6 | 三段式实盘周期链路(1m 单轮询+边界分发 5m/15m/30m/1h + 1d 14:30 快照 + 1w/1mon 启动通达信算) | **高**(回测=实盘硬要求,与切片5 无关) | C3+C5 已定规则 |
| G2 | 主循环轮询 /deals(按 `m_strOrderRef` 关联成交回填) | **高**(切片5 核心) | ✅ G3/G4 匹配键已定案 |
| G1 | 订单状态机(含提交时序:先写 submitted+commit 再 passorder) | 高 | ✅ 匹配键已定 |
| G6 | 拒单/部分成交持仓修正(ORDER status 54撤/56成 区分) | 高 | ✅ G3/G4 已定案 |
| E8 | 桥离线恢复不补 bar:BarPoller 加重置基线入口 + `_loop` 离线→在线转场检测 | 中(配合 C6 主循环改造) | ✅ 规则已定(E8 行) |
| B6 | 全局限 1 个实盘 session:`live.py:178` 改 `_ENGINES` 非空即拒新 start | 中(小改,防多引擎争抢桥) | ✅ 规则已定(B6 行) |
...
| D4 | 熔断计数读回 | 中 | — |
| H4 | 熔断计数持久化 | 中 | — |
| G7 | 桥状态并入 session API | 中 | — |
| F10 | 落库 status 拆 submitted | 中 | G1 |
| B5 | SSE 成交/信号推送 | 低 | — |
